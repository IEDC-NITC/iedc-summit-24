...
$sql = "SELECT `id`, `title` , `imageurl`, `priority` FROM `gallery` ";
...
while( $row = mysqli_fetch_row($queryRecords) ) { 
  ...
  if(!empty($row[1])){
     $thumbimage_data= 
    '<a href="'.$row[1].'" data-toggle="lightbox">
  ...
  $priority='<input type="number" style="max-width:80px;" class="form-control prioritychanger " grabtable="gallery" grabid="'.$row[0].'" value="'.$row[3].'"  />';
  array_push($table_data, $priority);
 

  $a='<a  data-toggle="modal" class="xc-viewedit-btn " xc-character="galleryDataLoad" data-target="#editGalleryModal" xc-edit="'.$row[0].'">View/Edit</a>  |';
  $a.='<a class="xcdelete"  href="./drop.php?id='.$row[0].'&type=gallery"> Delete</a>';
