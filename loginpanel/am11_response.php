...
$where = $sqlTot = $sqlRec = "";
$where .=" WHERE combination_key='RESULTS' ";
...
$sql = "SELECT `id`, `content` , `imageurl` FROM `sliders` ";
...
while( $row = mysqli_fetch_row($queryRecords) ) { 
 ...
 if( strlen($row[1]) > 50){
    $title = substr($row[1],0,50) .'...';
  }
  else{
    $title = $row[1];
  }
 
  array_push($table_data,$title);

  $a='<a  data-toggle="modal" class="xc-viewedit-btn " xc-character="sliderDataLoad" data-target="#editGalleryModal" xc-edit="'.$row[0].'">View/Edit</a>  |';
